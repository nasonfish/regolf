...
sub said{
  ...
  if($message->{channel} eq $channel and not $playing and $message->{body} =~ /^!start(?: !L ([a-zA-Z-]+))?$/){  # channel is correct, we're not already playing, the message starts with !start
    ...
    my $user = $message->{who};
    $user = $1 if $1;
    my $stats = db_user_stats($user);
    $self->say(channel => $channel, body => "$user stats: Scores between $stats[0] and $stats[1] averaging $stats[2] - $stats[3] wins and $stats[4] losses.");
