...
sub db_user_stats {
  my $user = $_[0];
  my $stmt = $db->prepare("SELECT MIN(score), MAX(score), AVG(score), COUNT(winner), (COUNT(user)-COUNT(winner)) FROM game_scores LEFT JOIN games ON game_scores.id = games.id WHERE user LIKE ?")->execute($user);
